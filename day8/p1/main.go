...
func encode(b []byte) int16 {
	return (int16(b[0]-'A')*26+int16(b[1]-'A'))*26 + int16(b[2]-'A')
}
...
func _run(scan *Scanner, bw *bufio.Writer) error {
	...
	for scan.Scan() {
		...
		nodes[node] = [2]int16{left, right}
	}

	start := encode([]byte("AAA"))
	finish := encode([]byte("ZZZ"))
	count := 0
	i := 0
