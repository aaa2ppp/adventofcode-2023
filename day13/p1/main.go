...
func searchAxis(nums []int) (int, bool) {
...
func readMirror(br *bufio.Reader) (rows, cols []int, err error) {
	for i := 0; ; i++ {
		...
		if cols == nil {
			if len(line) > 63 {
				return nil, nil, errors.New("line is longer than 63 characters")
			}
...
func calcLine(line []byte, i int, cols []int) int {
	row := 0
	for j, c := range line {
		if c == '#' {
			cols[j] |= 1 << i
