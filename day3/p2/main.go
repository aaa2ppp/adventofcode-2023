...
func _run(br *bufio.Reader, bw *bufio.Writer) error {
	...
	for i := range nums {
		checkNumber(&nums[i], simbols)
	}

	var sum int
	for _, g := range simbols {
		if g.n == 2 {
			sum += g.val
		}
	}
